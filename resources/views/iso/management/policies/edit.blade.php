    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-6 border-b border-gray-100">
                <h2 class="text-xl font-bold text-gray-800">Edit Policy</h2>
                <p class="text-sm text-gray-500 mt-1">Update the policy details, category categorization, and document link.</p>
            </div>
            ...
            <form action="{{ route('iso.management.policies.update', $policy->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="category_id" 
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('category_id', $policy->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="revision_count" 
                               id="revision_count" 
                               value="{{ old('revision_count', $policy->revision_count ?? 0) }}" 
                               min="0"
                               class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-950 focus:border-red-950 text-gray-800 shadow-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               ...
                               id="effectivity_date" 
                               value="{{ old('effectivity_date', $policy->effectivity_date ? \Carbon\Carbon::parse($policy->effectivity_date)->format('Y-m-d') : '') }}" 
                               class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-950 focus:border-red-950 text-gray-800 shadow-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" 
                               name="policy_date" 
                               id="policy_date" 
                               value="{{ old('policy_date', $policy->policy_date) }}" 
                ...
                <div>
                    ...
                    <textarea name="description" 
                              ...
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-950 focus:border-red-950 text-gray-800 shadow-sm">{{ old('description', $policy->description) }}</textarea>
                </div>

                <!-- Form Footer Buttons -->
                <div class="pt-6 border-t border-gray-100 flex flex-col sm:flex-row sm:justify-between items-center gap-4">
                    <!-- Left: Delete Button -->
                    <button type="submit" form="delete-policy-form" class="w-full sm:w-auto px-5 py-2.5 bg-white border border-red-200 text-red-600 rounded-xl text-sm font-semibold hover:bg-red-50 hover:text-red-700 transition-all flex items-center justify-center shadow-sm">
            ...
            <!-- Hidden Delete Form -->
            <form id="delete-policy-form" action="{{ route('iso.management.policies.destroy', $policy->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to permanently delete this policy? This action cannot be undone.');" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </div>
